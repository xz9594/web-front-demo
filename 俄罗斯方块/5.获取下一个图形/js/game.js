...
var class_game = {
	...
	_move_down:function(){
		//下边界检测
		if(!class_border._down()){//原图形已移动至底部,获取下一个图形
...
var class_border = {
	...
	_down: function(){
		for(var _i=0; _i<class_game._block.length; _i++){
			if(class_game._block[_i].y == (class_game._row-1) ){
				return false;
			}
		}
	...
	_right: function(){
		for(var _i=0; _i<class_game._block.length; _i++){
			if(class_game._block[_i].x == (class_game._column-1) ){
				return false;
			}
		}
