...
var class_game = {
	...
	_move:function(){
		...
		//判断边界
		//this._direction == "right" && _start_position%this._column==0
		//this._direction == "left" && (_start_position+1)%this._column==0
		//(_start_position<0 || _start_position>this._row*this._column)
		...
		//_init_position != _start_position 说明是运动状态,非静止
		...
		if(_start_position == _food_position){
			...
			this._food_y=Math.floor(Math.random()*this._row);//随机行数
